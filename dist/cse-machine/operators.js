...
function evaluateBinaryExpression(context, identifier, left, right) {
    ...
    if (left.type === 'string' && right.type === 'string' && identifier.name === '__py_adder') {
        if ((0, utils_1.isIdentifier)(identifier) && identifier.name === '__py_adder') {
            return {
                type: 'string',
        ...
        if (left.type !== right.type) {
            ...
            if (left.type === 'complex' || right.type === 'complex') {
                left.type = 'complex';
                right.type = 'complex';
                left.value = types_1.PyComplexNumber.fromValue(left.value);
                right.value = types_1.PyComplexNumber.fromValue(right.value);
            }
        ...
        if ((0, utils_1.isIdentifier)(identifier)) {
            if (identifier.name === '__py_adder') {
                if (left.type === 'complex' || right.type === 'complex') {
                    const leftComplex = types_1.PyComplexNumber.fromValue(left.value);
                    const rightComplex = types_1.PyComplexNumber.fromValue(right.value);
                ...
                ret_value = (0, utils_1.pythonMod)(left.value, right.value);
            }
